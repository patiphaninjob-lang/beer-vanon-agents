# Session Handoff - Ch34 AI Analysis Relocated to Chart Candles (v3.9.11)

- **v3.9.11 Fully Deployed**:
  ...
  * **NEWS Button Removal (New in v3.9.11)**: Completely removed the `NEWS` button section from the card templates on both US and Thai homepages to make the cards ultra-compact and clean.
  ...
  * **Version & Cache Bumps**: Bumped version tag to `v3.9.11` in `docs/index.html`, added the changelog details, and bumped `sw.js` cache names in `docs/sw.js` (to `v81`) and `docs/thai/sw.js` (to `v6`).
...
- `docs/thai/index.html`: Removed homepage card template analysis blocks and NEWS button, and added them to tooltips.
