# Session Handoff - Ch34 AI Analysis Relocated to Chart Candles (v3.9.12)

- **v3.9.12 Fully Deployed**:
  ...
  * **Thai Cards Homework Metadata (New in v3.9.12)**: Added the `homeworkTimeHtml` display showing the date/time of homework creation and session run phase (Premarket/Postmarket) to the top of Thai homepage stock cards.
  * **Thai History Homework Metadata (New in v3.9.12)**: Added the sidebar and tooltip daily archive metadata sections displaying the homework completed date/time and run phase to the Thai History Page.
  ...
  * **Version & Cache Bumps**: Bumped version tag to `v3.9.12` in `docs/index.html`, added the changelog details, and bumped `sw.js` cache names in `docs/sw.js` (to `v82`) and `docs/thai/sw.js` (to `v7`).
...
- `docs/thai/index.html`: Removed homepage card template analysis blocks and NEWS button, added `homeworkTimeHtml` headers, and added them to tooltips.
