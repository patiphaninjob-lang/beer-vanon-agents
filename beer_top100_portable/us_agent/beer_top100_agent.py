...
# Add dirs to sys.path so we can import shared modules
if str(COMMON_DIR) not in sys.path: sys.path.append(str(COMMON_DIR))
if str(SCRIPT_DIR) not in sys.path: sys.path.append(str(SCRIPT_DIR))

import json, smtplib, datetime, time, argparse, threading
import numpy as np
...
TOP_N            = 100
CALL_DELAY = 15.0  # ปรับเป็น 15 วินาที (4 ครั้ง/นาที) เพื่อให้รอดจาก 6000 TPM และทัน 30-min CI timeout
...
(DASHBOARD_DIR / "history-data").mkdir(parents=True, exist_ok=True)
...
def get_stock_context(ticker: str, rank: int, mktcap: float = 0, hist_df=None) -> dict:
    ...
    price_now  = float(clean_hist["Close"].iloc[-1]) if not clean_hist.empty else 0
...
def save_to_web(stocks_data: list, today: datetime.date, market_indices: dict = None, test_run: bool = False, market_news: dict = None) -> str:
...
def save_history_data(stocks_data: list, period: str = "5y") -> None:
    ...
    # Save Market Index History (Dow Jones: ^DJI, S&P 500: ^GSPC, NASDAQ: ^IXIC)
    indices = {"_DJI": "^DJI", "_SPX": "^GSPC", "_IXIC": "^IXIC"}
    for ticker_key, symbol in indices.items():
        try:
            safe_print(f"  ดึง history 1D สำหรับดัชนี {ticker_key} ({symbol})...")
            mkt_hist = yf.Ticker(symbol).history(period=period)
            if not mkt_hist.empty:
                ...
                (out_dir / f"{ticker_key}.json").write_text(
                    json.dumps(payload, ensure_ascii=False, separators=(",", ":")),
                    encoding="utf-8",
                )
                
                # Also save ^DJI as legacy market.json for compatibility
                if ticker_key == "_DJI":
                    (out_dir / "market.json").write_text(
                        json.dumps(payload, ensure_ascii=False, separators=(",", ":")),
                        encoding="utf-8",
                    )
                safe_print(f"  ✅ บันทึก {ticker_key}.json เรียบร้อย")
        except Exception as e:
            safe_print(f"  ⚠️ Failed to save market index history ({symbol}): {e}")
...
def main():
            ...
            for f in futures:
                ...
                if res:
                    ...
                    if not args.no_web:
                        ...
                        save_to_web(stocks_data, today, market_indices, test_run=args.test, market_news=market_news)
    ...
    if args.no_web:
        ...
        archive_url = save_to_web(stocks_data, today, market_indices, test_run=args.test, market_news=market_news)
