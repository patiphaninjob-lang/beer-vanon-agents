...
DATA_DIR        = DASHBOARD_DIR / "thai-data"
...
TOP_N            = 100
CALL_DELAY       = 15.0   # 15 วินาที เพื่อให้รอดจาก 6000 TPM (Token Per Minute)
GITHUB_PAGES_URL = "https://patiphaninjob-lang.github.io/beer-vanon-agents/thai"
...
(DASHBOARD_DIR / "thai-history-data").mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    if report_path.exists():
        try:
            ...
            if os.getenv("GITHUB_EVENT_NAME") == "schedule" and not today_data.get("test_run"):
                safe_print(f"⚠️ [Safety Net] ตรวจพบรายงานไทยของวันนี้ ({today_file_str}) แล้ว")
                return
