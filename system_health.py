"""
Lightweight canary checks for the online stock homework app.

The script writes docs/data/system_health.json so the dashboard can show
whether secrets and external providers are still usable.
"""
...
def check_yfinance() -> tuple[str, str]:
    ...
    if data is None or getattr(data, "empty", True):
        return "fail", "empty history response"

    available = []
    for ticker in ["SPY", "SPCX"]:
        try:
            frame = data[ticker]
    ...
    if "SPY" not in available:
        return "fail", "SPY history unavailable"
    if "SPCX" not in available:
        return "warning", "SPY ok; SPCX unavailable"
...
def check_archive_freshness(max_age_hours: int) -> tuple[str, str]:
    ...
    if not status_path.exists():
        return "warning", "docs/data/status.json missing"
    ...
    if age_hours > max_age_hours:
        return "warning", f"latest archive age {age_hours:.1f}h"
    return "ok", f"latest archive age {age_hours:.1f}h"
