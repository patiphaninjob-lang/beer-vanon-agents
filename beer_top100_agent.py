...
def save_to_web(
    ...
    if payload["health"]["status"] != "ok":
        safe_print(f"  ⚠️ archive health warning: {', '.join(payload['health']['issues'])}")
...
def main():
    ...
    for ticker, s_data in processed_tickers.items():
        ...
        if s_data.get("chart_b64"):
            try:
                chart_bytes = __import__("base64").b64decode(s_data["chart_b64"])
            except Exception:
                chart_bytes = b""
        if not chart_bytes:
            ...
            chart_bytes = generate_mini_chart_b64(ticker, hist_df=hist_df)
            if chart_bytes:
                safe_print(f"   [{s_data['rank']:3d}] {ticker:<8} -> เติมกราฟที่หายจากข้อมูลเดิม")
